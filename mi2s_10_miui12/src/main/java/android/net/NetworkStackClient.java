...
public class NetworkStackClient {
    ...
    public void start(Context context) {
        ...
        if (intent == null) {
            maybeCrashWithTerribleFailure("Could not resolve the network stack", context, (String) null);
            return;
        }
    ...
    public void maybeCrashWithTerribleFailure(String message, Context context, String packageName) {
        ...
        logWtf(str, (Throwable) null);
        long uptime = SystemClock.elapsedRealtime();
        long now = System.currentTimeMillis();
        long minCrashIntervalMs = DeviceConfig.getLong("connectivity", CONFIG_MIN_CRASH_INTERVAL_MS, DEFAULT_MIN_CRASH_INTERVAL_MS);
        ...
        if (alwaysCrash) {
        } else if (!justBooted && !haveKnownRecentCrash) {
            long j = uptime;
        } else if (str2 != null) {
            long j2 = uptime;
            synchronized (this.mHealthListeners) {
            ...
            while (it.hasNext()) {
                it.next().onNetworkStackFailure(str2);
            }
            return;
        } else {
            return;
    ...
    private INetworkStackConnector getRemoteConnector() {
        try {
            ...
            do {
                ...
                IBinder connector = service;
                if (service != null) {
                    return INetworkStackConnector.Stub.asInterface(connector);
                }
                Thread.sleep(20);
            } while (System.currentTimeMillis() - before <= JobStatus.DEFAULT_TRIGGER_UPDATE_DELAY);
            loge("Timeout waiting for NetworkStack connector", (Throwable) null);
            return null;
    ...
    public void dump(PrintWriter pw) {
        ...
        this.mLog.dump((FileDescriptor) null, pw, (String[]) null);
        synchronized (this.mPendingNetStackRequests) {
