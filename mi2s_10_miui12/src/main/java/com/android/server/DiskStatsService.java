...
public class DiskStatsService extends Binder {
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        if (DumpUtils.checkDumpAndUsageStatsPermission(this.mContext, TAG, printWriter)) {
            ...
            PrintWriter printWriter2 = pw4;
            reportFreeSpace(Environment.getDownloadCacheDirectory(), "Cache", printWriter2, proto, 1);
            reportFreeSpace(new File("/system"), "System", printWriter2, proto, 2);
            boolean fileBased = StorageManager.isFileEncryptedNativeOnly();
            if (!fileBased) {
                z = StorageManager.isBlockEncrypted();
            }
            boolean blockBased = z;
            if (!protoFormat) {
                proto2 = proto4;
                if (fileBased) {
                    pw3 = pw4;
    ...
    private void reportCachedValuesProto(ProtoOutputStream proto) {
        ...
        try {
            JSONObject json = new JSONObject(IoUtils.readFileAsString("/data/system/diskstats_cache.json"));
            long cachedValuesToken2 = protoOutputStream.start(1146756268038L);
            ...
            int length = appSizesArray.length();
            int len = packageNamesArray.length();
            if (len != length) {
                cachedValuesToken = cachedValuesToken2;
                JSONArray jSONArray = packageNamesArray;
            } else if (len != appDataSizesArray.length()) {
                cachedValuesToken = cachedValuesToken2;
                JSONArray jSONArray2 = packageNamesArray;
                ...
                cachedValuesToken = cachedValuesToken2;
                JSONArray jSONArray3 = packageNamesArray;
                protoOutputStream.end(cachedValuesToken);
            } else {
                cachedValuesToken = cachedValuesToken2;
                JSONArray jSONArray4 = packageNamesArray;
            }
