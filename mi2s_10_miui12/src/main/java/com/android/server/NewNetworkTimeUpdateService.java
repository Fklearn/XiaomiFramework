...
public class NewNetworkTimeUpdateService extends Binder implements NetworkTimeUpdateService {
    ...
    public NewNetworkTimeUpdateService(Context context) {
        ...
        this.mPollingIntervalMs = (long) this.mContext.getResources().getInteger(17694869);
        this.mPollingIntervalShorterMs = (long) this.mContext.getResources().getInteger(17694870);
        this.mTryAgainTimesMax = this.mContext.getResources().getInteger(17694871);
        this.mTimeErrorThresholdMs = this.mContext.getResources().getInteger(17694872);
        this.mWakeLock = ((PowerManager) context.getSystemService(PowerManager.class)).newWakeLock(1, TAG);
        NetworkTimeUpdateServiceInjector.initNtpServers(context, this.mTime);
    }
    ...
    private void onPollNetworkTimeUnderWakeLock(int event) {
        Log.d(TAG, "mNitzTimeSetTime: " + this.mNitzTimeSetTime + ", refTime: " + SystemClock.elapsedRealtime() + ", mLastNtpFetchTime: " + this.mLastNtpFetchTime);
        if (this.mTime.getCacheAge() >= this.mPollingIntervalMs) {
            Log.d(TAG, "Stale NTP fix; forcing refresh");
            NetworkTimeUpdateServiceInjector.switchNtpServer(this.mTryAgainCounter);
        }
    ...
    private class NetworkTimeUpdateCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onAvailable(Network network) {
            Log.d(NewNetworkTimeUpdateService.TAG, String.format("New default network %s; checking time.", new Object[]{network}));
