...
public class UpdateLockService extends IUpdateLock.Stub {
    ...
    public void sendLockChangedBroadcast(boolean state) {
        ...
        try {
            this.mContext.sendStickyBroadcastAsUser(new Intent("android.os.UpdateLock.UPDATE_LOCK_CHANGED").putExtra("nowisconvenient", state).putExtra(WatchlistLoggingHandler.WatchlistEventKeys.TIMESTAMP, System.currentTimeMillis()).addFlags(BroadcastQueueInjector.FLAG_IMMUTABLE), UserHandle.ALL);
        } finally {
