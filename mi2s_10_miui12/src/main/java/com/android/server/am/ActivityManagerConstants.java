...
final class ActivityManagerConstants extends ContentObserver {
    ...
    long CONTENT_PROVIDER_RETAIN_TIME = 20000;
    ...
    long POWER_CHECK_INTERVAL = BackupAgentTimeoutParameters.DEFAULT_FULL_BACKUP_AGENT_TIMEOUT_MILLIS;
    ...
    ActivityManagerConstants(Context context, ActivityManagerService service, Handler handler) {
        ...
        this.mSystemServerAutomaticHeapDumpPssThresholdBytes = Math.max(MIN_AUTOMATIC_HEAP_DUMP_PSS_THRESHOLD_BYTES, (long) context.getResources().getInteger(17694769));
        BoostFramework boostFramework = mPerf;
        if (boostFramework != null) {
            int intValue = Integer.valueOf(boostFramework.perfGetProp("ro.vendor.qti.sys.fw.bg_apps_limit", "32")).intValue();
            this.MAX_CACHED_PROCESSES = intValue;
            DEFAULT_MAX_CACHED_PROCESSES = intValue;
            USE_TRIM_SETTINGS = Boolean.parseBoolean(mPerf.perfGetProp("ro.vendor.qti.sys.fw.use_trim_settings", "true"));
            EMPTY_APP_PERCENT = Integer.valueOf(mPerf.perfGetProp("ro.vendor.qti.sys.fw.empty_app_percent", "50")).intValue();
            TRIM_EMPTY_PERCENT = Integer.valueOf(mPerf.perfGetProp("ro.vendor.qti.sys.fw.trim_empty_percent", "100")).intValue();
            TRIM_CACHE_PERCENT = Integer.valueOf(mPerf.perfGetProp("ro.vendor.qti.sys.fw.trim_cache_percent", "100")).intValue();
            TRIM_ENABLE_MEMORY = Long.valueOf(mPerf.perfGetProp("ro.vendor.qti.sys.fw.trim_enable_memory", "1073741824")).longValue();
        }
    }
    ...
    private void updateConstants() {
        ...
        synchronized (this.mService) {
            ...
            this.CONTENT_PROVIDER_RETAIN_TIME = this.mParser.getLong(KEY_CONTENT_PROVIDER_RETAIN_TIME, 20000);
    ...
    private void updateEnableAutomaticSystemServerHeapDumps() {
        ...
        this.mService.setDumpHeapDebugLimit((String) null, 0, enabled ? this.mSystemServerAutomaticHeapDumpPssThresholdBytes : 0, this.mSystemServerAutomaticHeapDumpPackageName);
    }
