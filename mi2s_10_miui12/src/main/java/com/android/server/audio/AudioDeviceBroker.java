...
final class AudioDeviceBroker {
    ...
    final Object mFixLock = new Object();
    ...
    public boolean setSpeakerphoneOn(boolean on, String eventSource) {
        ...
        synchronized (this.mDeviceStateLock) {
            ...
            this.mForcedUseForCommExt = this.mForcedUseForComm;
            Log.i(TAG, "In setSpeakerphoneOn(), mForcedUseForCommExt: " + this.mForcedUseForCommExt);
    ...
    public boolean isSpeakerphoneOn() {
        boolean z;
        synchronized (this.mDeviceStateLock) {
            Log.i(TAG, "In isSpeakerphoneOn(), mForcedUseForCommExt: " + this.mForcedUseForCommExt);
            z = true;
    ...
    /* access modifiers changed from: package-private */
    public void postBluetoothA2dpDeviceConfigChangeExt(BluetoothDevice device, int state, int profile, boolean suppressNoisyIntent, int a2dpVolume) {
        sendLMsgNoDelay(29, 2, new BtDeviceConnectionInfo(device, state, profile, suppressNoisyIntent, a2dpVolume));
    }
    ...
    public void setBluetoothScoOnByApp(boolean on) {
        synchronized (this.mDeviceStateLock) {
            ...
            } else if (this.mForcedUseForCommExt == 3) {
                this.mForcedUseForCommExt = 0;
            }
            Log.i(TAG, "In setBluetoothScoOnByApp(), mForcedUseForCommExt: " + this.mForcedUseForCommExt);
        }
    }
    ...
    public boolean isBluetoothScoOnForApp() {
        boolean z;
        synchronized (this.mDeviceStateLock) {
            Log.i(TAG, "In isBluetoothScoOnForApp(), mForcedUseForCommExt: " + this.mForcedUseForCommExt);
    ...
    public void setBluetoothScoOn(boolean on, String eventSource) {
        ...
        synchronized (this.mDeviceStateLock) {
            if (on) {
                if (this.mBtHelper.isBluetoothScoOn() || !this.mBtHelper.isBluetoothAudioNotConnectedToEarbud()) {
                    this.mForcedUseForComm = 3;
                } else {
                    Log.w(TAG, "setBluetoothScoOn(true) failed because device is not in audio connected mode");
                    this.mForcedUseForCommExt = 3;
                    return;
                }
            ...
            this.mForcedUseForCommExt = this.mForcedUseForComm;
            Log.i(TAG, "In setbluetoothScoOn(), mForcedUseForCommExt: " + this.mForcedUseForCommExt);
            AudioEventLogger audioEventLogger = AudioService.sDeviceLogger;
            audioEventLogger.log(new AudioEventLogger.StringEvent("setBluetoothScoOn = " + on + " " + eventSource + " uid/pid from " + Binder.getCallingUid() + SliceClientPermissions.SliceAuthority.DELIMITER + Binder.getCallingPid()));
    ...
    public void handleSetA2dpSinkConnectionState(int state, BtHelper.BluetoothA2dpDeviceInfo btDeviceInfo) {
        ...
        if (btDeviceInfo != null) {
            String address = btDeviceInfo.getBtDevice().getAddress();
        }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case AudioDeviceBroker.MSG_L_A2DP_DEVICE_CONNECTION_CHANGE_EXT /*27*/:
                    ...
                    synchronized (AudioDeviceBroker.this.mFixLock) {
                        synchronized (AudioDeviceBroker.this.mDeviceStateLock) {
                ...
                case 28:
                    ...
                    break;
                case 29:
                    BtDeviceConnectionInfo info3 = (BtDeviceConnectionInfo) msg.obj;
                    AudioEventLogger audioEventLogger3 = AudioService.sDeviceLogger;
                    audioEventLogger3.log(new AudioEventLogger.StringEvent("handleBluetoothA2dpActiveDeviceChangeExt  state=" + info3.mState + " addr=" + info3.mDevice.getAddress() + " prof=" + info3.mProfile + " supprNoisy=" + info3.mSupprNoisy + " vol=" + info3.mVolume).printLog(AudioDeviceBroker.TAG));
                    synchronized (AudioDeviceBroker.this.mDeviceStateLock) {
                        AudioDeviceBroker.this.mDeviceInventory.handleBluetoothA2dpActiveDeviceChangeExt(info3.mDevice, info3.mState, info3.mProfile, info3.mSupprNoisy, info3.mVolume);
                    }
                    break;
                case 30:
                    synchronized (AudioDeviceBroker.this.mSetModeLock) {
    ...
    public static boolean isMessageHandledUnderWakelock(int msgId) {
        if (msgId == 2 || msgId == 6 || msgId == 7 || msgId == 8 || msgId == 10 || msgId == 11 || msgId == 17 || msgId == 18) {
            return true;
        }
        switch (msgId) {
            case MSG_L_A2DP_DEVICE_CONNECTION_CHANGE_EXT /*27*/:
            case 28:
            case 29:
                return true;
            default:
    ...
    private void sendMsg(int msg, int existingMsgPolicy, int delay) {
        sendIILMsg(msg, existingMsgPolicy, 0, 0, (Object) null, delay);
    }
    ...
    private void sendIMsg(int msg, int existingMsgPolicy, int arg, int delay) {
        sendIILMsg(msg, existingMsgPolicy, arg, 0, (Object) null, delay);
    }

    private void sendMsgNoDelay(int msg, int existingMsgPolicy) {
        sendIILMsg(msg, existingMsgPolicy, 0, 0, (Object) null, 0);
    }

    private void sendIMsgNoDelay(int msg, int existingMsgPolicy, int arg) {
        sendIILMsg(msg, existingMsgPolicy, arg, 0, (Object) null, 0);
    }

    private void sendIIMsgNoDelay(int msg, int existingMsgPolicy, int arg1, int arg2) {
        sendIILMsg(msg, existingMsgPolicy, arg1, arg2, (Object) null, 0);
    }
