        ...
        if (UserBackupManagerService.RUN_INITIALIZE_ACTION.equals(intent.getAction())) {
            synchronized (this.mUserBackupManagerService.getQueueLock()) {
                ...
                if (pendingInits.size() > 0) {
                    ...
                    this.mUserBackupManagerService.getBackupHandler().post(new PerformInitializeTask(this.mUserBackupManagerService, (String[]) pendingInits.toArray(new String[pendingInits.size()]), (IBackupObserver) null, listener));
                }
            }
        }
    }
