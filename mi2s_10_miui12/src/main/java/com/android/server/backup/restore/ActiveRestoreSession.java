...
public class ActiveRestoreSession extends IRestoreSession.Stub {
    ...
    public synchronized int restorePackage(String packageName, IRestoreObserver observer, IBackupManagerMonitor monitor) {
        ...
        synchronized (this) {
            Slog.v(TAG, "restorePackage pkg=" + str + " obs=" + observer + "monitor=" + monitor);
            if (this.mEnded) {
                throw new IllegalStateException("Restore session already ended");
            } else if (this.mTimedOut) {
                Slog.i(TAG, "Session already timed out");
                ...
                try {
                    PackageInfo app = this.mBackupManagerService.getPackageManager().getPackageInfoAsUser(str, 0, this.mUserId);
                    ...
                    try {
                        ...
                        $$Lambda$ActiveRestoreSession$tb1mCMujBEuhHsxQ6tX_mYJVCII r5 = new BiFunction(observer, monitor, token, app) {
                            private final /* synthetic */ IRestoreObserver f$0;
                            private final /* synthetic */ IBackupManagerMonitor f$1;
                            private final /* synthetic */ long f$2;
