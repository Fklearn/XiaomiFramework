...
public class TransportClient {
    ...
    public void connectAsync(TransportConnectionListener listener, String caller) {
        synchronized (this.mStateLock) {
            ...
            if (i == 0) {
                ...
                notifyListener(listener, (IBackupTransport) null, caller);
            } else if (i != 1) {
                if (i == 2) {
                ...
                setStateLocked(2, (IBackupTransport) null);
                this.mListeners.put(listener, caller);
            } else {
    ...
    public void unbind(String caller) {
        synchronized (this.mStateLock) {
            ...
            if (!(i == 0 || i == 1)) {
                if (i == 2) {
                    setStateLocked(1, (IBackupTransport) null);
                    this.mContext.unbindService(this.mConnection);
                    notifyListenersAndClearLocked((IBackupTransport) null);
                } else if (i == 3) {
                    setStateLocked(1, (IBackupTransport) null);
                    this.mContext.unbindService(this.mConnection);
                }
    ...
    public void onServiceDisconnected() {
        synchronized (this.mStateLock) {
            ...
            setStateLocked(0, (IBackupTransport) null);
            try {
                this.mContext.unbindService(this.mConnection);
    ...
    private void log(int priority, String message) {
        TransportUtils.log(priority, TAG, TransportUtils.formatMessage(this.mPrefixForLog, (String) null, message));
        saveLogEntry(TransportUtils.formatMessage((String) null, (String) null, message));
    }
    ...
    private void log(int priority, String caller, String message) {
        ...
        saveLogEntry(TransportUtils.formatMessage((String) null, caller, message));
    }

    private void saveLogEntry(String message) {
        String message2 = DateFormat.format("yyyy-MM-dd HH:mm:ss", System.currentTimeMillis()) + " " + message;
