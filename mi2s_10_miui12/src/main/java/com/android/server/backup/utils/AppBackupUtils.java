...
public class AppBackupUtils {
    ...
    static boolean appIsEligibleForBackup(ApplicationInfo app, PackageManagerInternal packageManager, int userId) {
        ...
        if (Build.IS_INTERNATIONAL_BUILD || !appIsDisabled(app, packageManager, userId)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean appGetsFullBackup(PackageInfo pkg) {
        if (pkg.applicationInfo.backupAgentName == null || (pkg.applicationInfo.flags & BroadcastQueueInjector.FLAG_IMMUTABLE) != 0) {
            return true;
        }
        return false;
    }
    ...
    public static boolean signaturesMatch(Signature[] storedSigs, PackageInfo target, PackageManagerInternal pmi) {
        ...
        if ((target.applicationInfo.flags & 1) != 0) {
            Slog.v(BackupManagerService.TAG, "System app " + target.packageName + " - skipping sig check");
            return true;
        } else if (ArrayUtils.isEmpty(storedSigs)) {
            return false;
        } else {
