...
public abstract class ClientMonitor extends LoggableMonitor implements IBinder.DeathRecipient {
    ...
    private boolean blacklistContains(int acquiredInfo, int vendorCode) {
        if (acquiredInfo == this.mConstants.acquireVendorCode()) {
            for (int i : getAcquireVendorIgnorelist()) {
                if (i == vendorCode) {
                    Slog.v(getLogTag(), "Ignoring vendor message: " + vendorCode);
                    return true;
                }
            }
            return false;
        }
        for (int i2 : getAcquireIgnorelist()) {
            if (i2 == acquiredInfo) {
                Slog.v(getLogTag(), "Ignoring message: " + acquiredInfo);
                return true;
            }
        }
        return false;
