...
public class LingerMonitor {
    ...
    public boolean isNotificationEnabled(NetworkAgentInfo fromNai, NetworkAgentInfo toNai) {
        for (String notifySwitch : this.mContext.getResources().getStringArray(17236044)) {
    ...
    public PendingIntent createNotificationIntent() {
        return PendingIntent.getActivityAsUser(this.mContext, 0, CELLULAR_SETTINGS, 268435456, (Bundle) null, UserHandle.CURRENT);
    }
    ...
    private void notify(NetworkAgentInfo fromNai, NetworkAgentInfo toNai, boolean forceToast) {
        int notifyType = this.mContext.getResources().getInteger(17694855);
