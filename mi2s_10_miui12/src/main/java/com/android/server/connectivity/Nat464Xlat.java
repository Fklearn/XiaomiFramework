...
public class Nat464Xlat extends BaseNetworkObserver {
    ...
    protected static boolean requiresClat(NetworkAgentInfo nai) {
        int type = nai.networkInfo.getType();
        ...
        String xlatConfigValue = NetPluginDelegate.getConfig(xlatRequired, "true");
        if (xlatConfigValue != null && xlatConfigValue.equals("true")) {
            String str = TAG;
            Slog.e(str, "getConfig: For config := persist.vendor.net.doxlat returned value := " + xlatConfigValue);
            androidXlatEnabled = true;
        }
        if (!supported || !connected || !isIpv6OnlyNetwork || skip464xlat || !androidXlatEnabled) {
            return false;
        }
        return true;
    }
    ...
    public void lambda$interfaceLinkStateChanged$0$Nat464Xlat(String iface, boolean up) {
        if (isStarting() && up && Objects.equals(this.mIface, iface)) {
            ...
            Slog.i(str2, String.format("interface %s is up, adding stacked link %s on top of %s", new Object[]{str3, str3, this.mBaseIface}));
            enterRunningState();
            LinkProperties lp = new LinkProperties(this.mNetwork.linkProperties);
