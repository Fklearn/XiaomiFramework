...
public class EntitlementManager {
    ...
    private static final ComponentName TETHER_SERVICE = ComponentName.unflattenFromString(Resources.getSystem().getString(17039807));
    ...
    public EntitlementManager(Context ctx, StateMachine tetherMasterSM, SharedLog log, int permissionChangeMessageCode, MockableSystemProperties systemProperties) {
        ...
        this.mContext.registerReceiver(this.mReceiver, new IntentFilter(ACTION_PROVISIONING_ALARM), (String) null, this.mHandler);
    }
    ...
    public void runSilentTetherProvisioning(int type, int subId) {
        ResultReceiver receiver = buildProxyReceiver(type, false, (ResultReceiver) null);
    ...
    private void runUiTetherProvisioning(int type, int subId) {
        runUiTetherProvisioning(type, subId, buildProxyReceiver(type, true, (ResultReceiver) null));
    }
    ...
    private void scheduleProvisioningRechecks(TetheringConfiguration config) {
        ...
        if (this.mProvisioningRecheckAlarm == null && (period = config.provisioningCheckPeriod) > 0) {
            ...
            ((AlarmManager) this.mContext.getSystemService("alarm")).setRepeating(3, firstAlarmTime, periodMs, this.mProvisioningRecheckAlarm);
        }
    }
    ...
    private static String typeString(int type) {
        ...
        return String.format("TETHERING UNKNOWN TYPE (%d)", new Object[]{Integer.valueOf(type)});
    }
    ...
    private static String errorString(int value) {
        ...
        return String.format("UNKNOWN ERROR (%d)", new Object[]{Integer.valueOf(value)});
    }

    private ResultReceiver buildProxyReceiver(int type, boolean notifyFail, ResultReceiver receiver) {
        final int i = type;
        final boolean z = notifyFail;
        final ResultReceiver resultReceiver = receiver;
        return writeToParcel(new ResultReceiver(this.mHandler) {
            /* access modifiers changed from: protected */
            public void onReceiveResult(int resultCode, Bundle resultData) {
                int updatedCacheValue = EntitlementManager.this.updateEntitlementCacheValue(i, resultCode);
                EntitlementManager.this.addDownstreamMapping(i, updatedCacheValue);
                if (updatedCacheValue == 11 && z) {
                    EntitlementManager.this.mListener.onUiEntitlementFailed(i);
                }
                ResultReceiver resultReceiver = resultReceiver;
                if (resultReceiver != null) {
                    resultReceiver.send(updatedCacheValue, (Bundle) null);
                }
            }
        });
    }
    ...
    public void handleGetLatestTetheringEntitlementValue(int downstream, ResultReceiver receiver, boolean showEntitlementUi) {
        ...
        if (cacheValue == 0 || !showEntitlementUi) {
            receiver.send(cacheValue, (Bundle) null);
            return;
        }
