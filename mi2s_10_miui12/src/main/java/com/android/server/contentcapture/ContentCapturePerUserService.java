...
final class ContentCapturePerUserService extends AbstractPerUserSystemService<ContentCapturePerUserService, ContentCaptureManagerService> implements RemoteContentCaptureService.ContentCaptureServiceCallbacks {
    ...
    public void onConnected() {
        /*
            ...
            monitor-enter(r0)
            boolean r1 = r3.mZombie     // Catch:{ all -> 0x001c }
            if (r1 == 0) goto L_0x001a
            com.android.server.contentcapture.RemoteContentCaptureService r1 = r3.mRemoteService     // Catch:{ all -> 0x001c }
            if (r1 != 0) goto L_0x0014
            java.lang.String r1 = TAG     // Catch:{ all -> 0x001c }
            java.lang.String r2 = "Cannot ressurect sessions because remote service is null"
            android.util.Slog.w(r1, r2)     // Catch:{ all -> 0x001c }
            monitor-exit(r0)     // Catch:{ all -> 0x001c }
            return
        L_0x0014:
            r1 = 0
            r3.mZombie = r1     // Catch:{ all -> 0x001c }
            r3.resurrectSessionsLocked()     // Catch:{ all -> 0x001c }
        L_0x001a:
            monitor-exit(r0)     // Catch:{ all -> 0x001c }
            return
        L_0x001c:
            ...
            throw r1
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.contentcapture.ContentCapturePerUserService.onConnected():void");
    }
    ...
    public void startSessionLocked(IBinder activityToken, ActivityPresentationInfo activityPresentationInfo, int sessionId, int uid, int flags, IResultReceiver clientReceiver) {
        ...
        ActivityPresentationInfo activityPresentationInfo2 = activityPresentationInfo;
        int i = sessionId;
        IResultReceiver iResultReceiver = clientReceiver;
        if (activityPresentationInfo2 == null) {
            Slog.w(TAG, "basic activity info is null");
            ContentCaptureService.setClientState(iResultReceiver, 260, (IBinder) null);
            return;
        }
        int taskId = activityPresentationInfo2.taskId;
        int displayId = activityPresentationInfo2.displayId;
        ComponentName componentName = activityPresentationInfo2.componentName;
        ...
        if (!enabled) {
            ...
            ContentCaptureService.setClientState(iResultReceiver, UsbTerminalTypes.TERMINAL_IN_OMNI_MIC, (IBinder) null);
            ...
            ContentCaptureServerSession existingSession = this.mSessions.get(i);
            if (existingSession != null) {
                String str3 = TAG;
                Slog.w(str3, "startSession(id=" + existingSession + ", token=" + iBinder + ": ignoring because it already exists for " + existingSession.mActivityToken);
                ContentCaptureService.setClientState(iResultReceiver, 12, (IBinder) null);
                ContentCaptureServerSession contentCaptureServerSession = existingSession;
                ContentCaptureMetricsLogger.writeSessionEvent(sessionId, 3, 12, serviceComponentName, componentName, false);
                return;
            }
            ContentCaptureServerSession existingSession2 = existingSession;
            if (this.mRemoteService == null) {
            ...
            if (remoteContentCaptureService == null) {
                String str4 = TAG;
                Slog.w(str4, "startSession(id=" + existingSession2 + ", token=" + iBinder + ": ignoring because service is not set");
                ContentCaptureService.setClientState(iResultReceiver, 20, (IBinder) null);
                ContentCaptureMetricsLogger.writeSessionEvent(sessionId, 3, 20, serviceComponentName, componentName, false);
                return;
            }
            remoteContentCaptureService.ensureBoundLocked();
            boolean z = whiteListed;
            ...
            if (((ContentCaptureManagerService) this.mMaster).verbose) {
                String str5 = TAG;
                Slog.v(str5, "startSession(): new session for " + ComponentName.flattenToShortString(componentName2) + " and id " + i);
            }
            this.mSessions.put(i, newSession);
            newSession.notifySessionStartedLocked(iResultReceiver);
        }
    }
