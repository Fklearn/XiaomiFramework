...
final class LogicalDisplay {
    ...
    private static Rect getMaskingInsets(DisplayDeviceInfo deviceInfo) {
        ...
        Rect insets = deviceInfo.displayCutout.getSafeInsets();
        if (insets.left % 2 != 0) {
            insets.left++;
        }
        if (insets.right % 2 != 0) {
    ...
    public void configureDisplayLocked(SurfaceControl.Transaction t, DisplayDevice device, boolean isBlanked) {
        ...
        if (displayDevice == this.mPrimaryDisplayDevice) {
            displayDevice.setAllowedDisplayModesLocked(this.mAllowedDisplayModes);
            displayDevice.setRequestedColorModeLocked(this.mRequestedColorMode);
        } else {
            displayDevice.setAllowedDisplayModesLocked(new int[]{0});
