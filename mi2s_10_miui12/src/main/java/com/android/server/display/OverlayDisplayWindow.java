...
final class OverlayDisplayWindow implements DumpUtils.Dump {
    ...
    private void resize(int width, int height, int densityDpi, boolean doLayout) {
        ...
        this.mTitle = this.mContext.getResources().getString(17039909, new Object[]{this.mName, Integer.valueOf(this.mWidth), Integer.valueOf(this.mHeight), Integer.valueOf(this.mDensityDpi)});
        if (this.mSecure) {
            this.mTitle += this.mContext.getResources().getString(17039908);
        }
        if (doLayout) {
            relayout();
        }
    ...
    private void createWindow() {
        this.mWindowContent = LayoutInflater.from(this.mContext).inflate(17367241, (ViewGroup) null);
