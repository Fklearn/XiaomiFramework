...
final class WifiDisplayAdapter extends DisplayAdapter {
    ...
    public WifiDisplayAdapter(DisplayManagerService.SyncRoot syncRoot, Context context, Handler handler, DisplayAdapter.Listener listener, PersistentDataStore persistentDataStore) {
        ...
        this.mSupportsProtectedBuffers = context.getResources().getBoolean(17891579);
    }
    ...
    public void registerLocked() {
        ...
        getHandler().post(new Runnable() {
            public void run() {
                ...
                WifiDisplayAdapter.this.getContext().registerReceiverAsUser(WifiDisplayAdapter.this.mBroadcastReceiver, UserHandle.ALL, new IntentFilter(WifiDisplayAdapter.ACTION_DISCONNECT), (String) null, WifiDisplayAdapter.this.mHandler);
            }
        });
    }
    ...
    public void updateDisplaysLocked() {
        ...
        for (WifiDisplay d : this.mRememberedDisplays) {
            ...
            while (true) {
                ...
                if (i >= wifiDisplayArr.length) {
                    break;
                } else if (d.equals(wifiDisplayArr[i])) {
                    available = true;
                    remembered[i] = true;
                    break;
                } else {
        ...
        while (true) {
            ...
            if (i2 < wifiDisplayArr2.length) {
                WifiDisplay d2 = wifiDisplayArr2[i2];
