...
final class DisplayWhiteBalanceTintController extends TintController {
    ...
    public void setUp(Context context, boolean needsLinear) {
        ...
        if (!isColorMatrixValid(displayColorSpaceRGB.getTransform())) {
            ...
            String[] nominalWhiteValues = res.getStringArray(17236017);
    ...
    private String matrixToString(float[] matrix, int columns) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            ...
            sb.append(String.format("%9.6f", new Object[]{Float.valueOf(matrix[i])}));
        }
        return sb.toString();
    }
    ...
    private ColorSpace.Rgb getDisplayColorSpaceFromResources(Resources res) {
        String[] displayPrimariesValues = res.getStringArray(17236018);
    ...
    private boolean isColorMatrixValid(float[] matrix) {
        ...
        for (float isColorMatrixCoeffValid : matrix) {
            if (!isColorMatrixCoeffValid(isColorMatrixCoeffValid)) {
                return false;
            }
        }
        return true;
