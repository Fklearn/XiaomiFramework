...
class PendingReports {
    ...
    private final class PendingReportRec {
        ...
        public Uri getUri() {
            Uri.Builder builder = new Uri.Builder().scheme(ActivityTaskManagerInternal.ASSIST_KEY_CONTENT).authority("android.os.IncidentManager").path("/pending").appendQueryParameter("id", Integer.toString(this.id)).appendQueryParameter(SplitScreenReporter.STR_PKG, this.callingPackage).appendQueryParameter("flags", Integer.toString(this.flags)).appendQueryParameter("t", Long.toString(this.addedWalltime));
        ...
        if (args.length == 0) {
            ...
            synchronized (this.mLock) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    writer.println(String.format("  %11d %s: %s", new Object[]{Long.valueOf(entry.addedRealtime), df.format(new Date(entry.addedWalltime)), entry.getUri().toString()}));
                }
            }
        }
    }
    ...
    public void lambda$authorizeReport$0$PendingReports(int callingUid, String callingPackage, String receiverClass, String reportId, int flags, IIncidentAuthListener listener) {
        ...
        IIncidentAuthListener iIncidentAuthListener = listener;
        if (i == 0 || isPackageInUid(callingUid, callingPackage)) {
            ...
            try {
                listener.asBinder().linkToDeath(new IBinder.DeathRecipient(iIncidentAuthListener, receiver, primaryUser) {
                    private final /* synthetic */ IIncidentAuthListener f$1;
                ...
                cancelReportImpl(iIncidentAuthListener, receiver, primaryUser);
            }
