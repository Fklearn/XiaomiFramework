...
public abstract class AbstractMasterSystemService<M extends AbstractMasterSystemService<M, S>, S extends AbstractPerUserSystemService<S, M>> extends SystemService {
    ...
    public S getServiceForUserLocked(int userId) {
        int resolvedUserId = ActivityManager.handleIncomingUser(Binder.getCallingPid(), Binder.getCallingUid(), userId, false, false, (String) null, (String) null);
        S service = (AbstractPerUserSystemService) this.mServicesCache.get(resolvedUserId);
        if (service == null) {
    ...
    public S peekServiceForUserLocked(int userId) {
        return (AbstractPerUserSystemService) this.mServicesCache.get(ActivityManager.handleIncomingUser(Binder.getCallingPid(), Binder.getCallingUid(), userId, false, false, (String) null, (String) null));
    }
        ...
        try {
            ...
            if (size == 0) {
                ...
                for (int i2 = 0; i2 < size; i2++) {
                    ...
                    pw.print(i2);
                    pw.println(": ");
                    ((AbstractPerUserSystemService) this.mServicesCache.valueAt(i2)).dumpLocked("    ", pw);
                    pw.println();
                }
            }
        } finally {
        ...
        new PackageMonitor() {
            ...
            private void handleActiveServiceRemoved(int userId) {
                ...
                if (serviceSettingsProperty != null) {
                    Settings.Secure.putStringForUser(AbstractMasterSystemService.this.getContext().getContentResolver(), serviceSettingsProperty, (String) null, userId);
                }
            }
