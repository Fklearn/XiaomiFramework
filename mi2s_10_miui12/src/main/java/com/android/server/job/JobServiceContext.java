...
import com.android.server.am.JobServiceContextInjector;
...
public final class JobServiceContext implements ServiceConnection {
    ...
    public boolean executeRunnableJob(JobStatus job) {
        ...
        synchronized (this.mLock) {
            ...
            if (jobStatus.changedUris != null) {
                Uri[] triggeredUris2 = new Uri[jobStatus.changedUris.size()];
                jobStatus.changedUris.toArray(triggeredUris2);
                triggeredUris = triggeredUris2;
            } else {
                triggeredUris = null;
            }
            if (jobStatus.changedAuthorities != null) {
                String[] triggeredAuthorities2 = new String[jobStatus.changedAuthorities.size()];
                jobStatus.changedAuthorities.toArray(triggeredAuthorities2);
                triggeredAuthorities = triggeredAuthorities2;
            } else {
                triggeredAuthorities = null;
            }
            JobInfo ji = job.getJob();
            this.mParams = new JobParameters(this.mRunningCallback, job.getJobId(), ji.getExtras(), ji.getTransientExtras(), ji.getClipData(), ji.getClipGrantFlags(), isDeadlineExpired, triggeredUris, triggeredAuthorities, jobStatus.network);
            ...
            try {
                binding = JobServiceContextInjector.bindService(this, this.mContext, new Intent().setComponent(job.getServiceComponent()), jobStatus);
    ...
    public void doAcknowledgeStopMessage(JobCallback cb, int jobId, boolean reschedule) {
        doCallback(cb, reschedule, (String) null);
    }
    ...
    public android.app.job.JobWorkItem doDequeueWork(com.android.server.job.JobServiceContext.JobCallback r7, int r8) {
        /*
            r6 = this;
            long r0 = android.os.Binder.clearCallingIdentity()
            java.lang.Object r2 = r6.mLock     // Catch:{ all -> 0x003a }
            monitor-enter(r2)     // Catch:{ all -> 0x003a }
            r6.assertCallerLocked(r7)     // Catch:{ all -> 0x0037 }
            int r3 = r6.mVerb     // Catch:{ all -> 0x0037 }
            r4 = 3
            ...
            goto L_0x0031
        L_0x0015:
            com.android.server.job.controllers.JobStatus r3 = r6.mRunningJob     // Catch:{ all -> 0x0037 }
            android.app.job.JobWorkItem r3 = r3.dequeueWorkLocked()     // Catch:{ all -> 0x0037 }
            if (r3 != 0) goto L_0x002c
            com.android.server.job.controllers.JobStatus r4 = r6.mRunningJob     // Catch:{ all -> 0x0037 }
            boolean r4 = r4.hasExecutingWorkLocked()     // Catch:{ all -> 0x0037 }
            if (r4 != 0) goto L_0x002c
            r4 = 0
            java.lang.String r5 = "last work dequeued"
            r6.doCallbackLocked(r4, r5)     // Catch:{ all -> 0x0037 }
        L_0x002c:
    ...
    private WorkSource deriveWorkSource(JobStatus runningJob) {
        ...
        if (!WorkSource.isChainedBatteryAttributionEnabled(this.mContext)) {
            return new WorkSource(jobUid, runningJob.getSourcePackageName());
        }
    ...
    private void handleStartedLocked(boolean workOngoing) {
        ...
        if (!workOngoing) {
            ...
            handleCancelLocked((String) null);
        } else {
            scheduleOpTimeOutLocked();
        }
    }
