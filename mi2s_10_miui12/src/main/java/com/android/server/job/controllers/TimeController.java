...
public final class TimeController extends StateController {
    ...
    public void maybeStartTrackingJobLocked(JobStatus job, JobStatus lastJob) {
        if (job.hasTimingDelayConstraint() || job.hasDeadlineConstraint()) {
            maybeStopTrackingJobLocked(job, (JobStatus) null, false);
    ...
    private void updateAlarmWithListenerLocked(String tag, AlarmManager.OnAlarmListener listener, long alarmTimeElapsed, WorkSource ws) {
        ...
        if (DEBUG) {
            Slog.d(TAG, "Setting " + tag + " for: " + j);
        } else {
            String str = tag;
        }
        this.mAlarmService.set(2, alarmTimeElapsed, -1, 0, tag, listener, MiuiFgThread.getHandler(), ws);
    }
