...
public class SyntheticPasswordCrypto {
    ...
    public static byte[] decryptBlobV1(String keyAlias, byte[] blob, byte[] applicationId) {
        try {
            ...
            return decrypt((SecretKey) keyStore.getKey(keyAlias, (char[]) null), decrypt(applicationId, APPLICATION_ID_PERSONALIZATION, blob));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static byte[] decryptBlob(String keyAlias, byte[] blob, byte[] applicationId) {
        try {
            KeyStore keyStore = KeyStore.getInstance("AndroidKeyStore");
            keyStore.load((KeyStore.LoadStoreParameter) null);
            return decrypt(applicationId, APPLICATION_ID_PERSONALIZATION, decrypt((SecretKey) keyStore.getKey(keyAlias, (char[]) null), blob));
