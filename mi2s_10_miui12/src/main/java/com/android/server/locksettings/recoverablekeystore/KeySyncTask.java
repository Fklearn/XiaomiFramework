...
public class KeySyncTask implements Runnable {
    ...
    private void syncKeysForAgent(int recoveryAgentUid) throws IOException, RemoteException {
        ...
        if (!shouldCreateSnapshot(recoveryAgentUid)) {
            boolean shouldRecreateCurrentVersion2 = this.mRecoverableKeyStoreDb.getSnapshotVersion(this.mUserId, i) != null && this.mRecoverySnapshotStorage.get(i) == null;
            ...
            try {
                ...
                try {
                    ...
                    if (this.mCredentialUpdated) {
                        ...
                        Long counterId2 = this.mRecoverableKeyStoreDb.getCounterId(this.mUserId, i);
                        if (counterId2 == null) {
                            counterId = Long.valueOf(generateAndStoreCounterId(recoveryAgentUid));
                        } else {
                    ...
                    try {
                        ...
                        byte[] bArr3 = salt;
                        byte[] bArr4 = localLskfHash;
                        KeyChainSnapshot.Builder keyChainSnapshotBuilder = new KeyChainSnapshot.Builder().setSnapshotVersion(getSnapshotVersion(i, shouldRecreateCurrentVersion)).setMaxAttempts(10).setCounterId(counterId.longValue()).setServerParams(vaultHandle).setKeyChainProtectionParams(arrayList).setWrappedApplicationKeys(createApplicationKeyEntries(encryptedApplicationKeys, rawKeysWithMetadata)).setEncryptedRecoveryKeyBlob(encryptedRecoveryKey);
                        try {
                            ...
                            Log.wtf(str, "Cannot serialize CertPath when calling setTrustedHardwareCertPath", e);
                        }
                    } catch (NoSuchAlgorithmException e2) {
                        byte[] bArr5 = vaultParams;
                        PublicKey publicKey3 = publicKey;
                        boolean z2 = useScryptToHashCredential;
                        byte[] vaultParams2 = salt;
                        byte[] bArr6 = localLskfHash;
                        Log.wtf(str, "SecureBox encrypt algorithms unavailable", e2);
                    } catch (InvalidKeyException e3) {
                        byte[] bArr7 = vaultParams;
                        PublicKey publicKey4 = publicKey;
                        boolean z3 = useScryptToHashCredential;
                        byte[] vaultParams3 = salt;
                        byte[] bArr8 = localLskfHash;
                        Log.e(str, "Could not encrypt with recovery key", e3);
                    }
