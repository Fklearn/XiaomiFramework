...
public class RecoverableKeyGenerator {
    ...
    public void importKey(PlatformEncryptionKey platformKey, int userId, int uid, String alias, byte[] keyBytes, byte[] metadata) throws RecoverableKeyStorageException, KeyStoreException, InvalidKeyException {
        ...
        int i2 = uid;
        String str = alias;
        if (this.mDatabase.insertKey(i, i2, str, WrappedKey.fromSecretKey(platformKey, new SecretKeySpec(keyBytes, SECRET_KEY_ALGORITHM), metadata)) != -1) {
            this.mDatabase.setShouldCreateSnapshot(i, i2, true);
            return;
        }
        throw new RecoverableKeyStorageException(String.format(Locale.US, "Failed writing (%d, %s) to database.", new Object[]{Integer.valueOf(uid), str}));
    }
}
