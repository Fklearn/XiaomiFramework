...
public class LockdownVpnTracker {
    ...
    private void handleStateChangedLocked() {
        ...
        Slog.d(TAG, "handleStateChanged: egress=" + egressTypeName + " " + this.mAcceptedEgressIface + "->" + (egressProp == null ? null : egressProp.getInterfaceName()));
        ...
        if (this.mErrorCount > 4) {
            showNotification(17041332, 17303830);
            ...
            if (vpnInfo.isConnected() && vpnConfig != null) {
                ...
                if (!TextUtils.equals(iface, this.mAcceptedIface) || !sourceAddrs.equals(this.mAcceptedSourceAddr)) {
                    ...
                    showNotification(17041329, 17303829);
            ...
            showNotification(17041330, 17303830);
    ...
    private void initLocked() {
        ...
        this.mContext.registerReceiver(this.mResetReceiver, new IntentFilter(ACTION_LOCKDOWN_RESET), "android.permission.CONNECTIVITY_INTERNAL", (Handler) null);
    ...
    public void augmentNetworkInfo(NetworkInfo info) {
        if (info.isConnected()) {
            NetworkInfo vpnInfo = this.mVpn.getNetworkInfo();
            info.setDetailedState(vpnInfo.getDetailedState(), vpnInfo.getReason(), (String) null);
        }
    }

    private void showNotification(int titleRes, int iconRes) {
        NotificationManager.from(this.mContext).notify((String) null, 20, new Notification.Builder(this.mContext, SystemNotificationChannels.VPN).setWhen(0).setSmallIcon(iconRes).setContentTitle(this.mContext.getString(titleRes)).setContentText(this.mContext.getString(17041328)).setContentIntent(this.mConfigIntent).setOngoing(true).addAction(17302726, this.mContext.getString(17041017), this.mResetIntent).setColor(this.mContext.getColor(17170460)).build());
    }

    private void hideNotification() {
        NotificationManager.from(this.mContext).cancel((String) null, 20);
    }
}
