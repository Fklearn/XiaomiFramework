...
public class NetworkIdentitySet extends HashSet<NetworkIdentity> implements Comparable<NetworkIdentitySet> {
    ...
    public NetworkIdentitySet(DataInputStream in) throws IOException {
        ...
        while (i < size) {
            ...
            NetworkIdentity networkIdentity = r4;
            NetworkIdentity networkIdentity2 = new NetworkIdentity(type, subType, subscriberId, networkId, roaming, metered, defaultNetwork);
            add(networkIdentity);
            i++;
            version = version2;
        }
