    ...
    public static boolean isAccessibleToUser(int uid, int callerUid, int accessLevel) {
        if (accessLevel != 1) {
            if (accessLevel != 2) {
                ...
                return false;
            } else if (uid == 1000 || uid == -4 || uid == -5 || uid == -1 || UserHandle.getUserId(uid) == UserHandle.getUserId(callerUid)) {
                return true;
            } else {
                return false;
            }
        } else if (uid == 1000 || uid == -4 || uid == -5 || UserHandle.getUserId(uid) == UserHandle.getUserId(callerUid)) {
            return true;
        } else {
            return false;
        }
