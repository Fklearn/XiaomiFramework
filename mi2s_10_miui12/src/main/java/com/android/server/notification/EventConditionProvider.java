...
public class EventConditionProvider extends SystemConditionProviderService {
    ...
    public void reloadTrackers() {
        ...
        for (int i = 0; i < this.mTrackers.size(); i++) {
            this.mTrackers.valueAt(i).setCallback((CalendarTracker.Callback) null);
        }
    ...
    private void rescheduleAlarm(long now, long time) {
        ...
        PendingIntent pendingIntent = PendingIntent.getBroadcast(this.mContext, 1, new Intent(ACTION_EVALUATE).addFlags(268435456).putExtra("time", time), 134217728);
