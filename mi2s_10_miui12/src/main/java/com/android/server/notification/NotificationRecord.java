...
public final class NotificationRecord {
    ...
    private long mLastAudiblyAlertedMs = -1;
    ...
    private boolean mStatusBarCheckedImportance;
    ...
    private Light calculateLights() {
        ...
        Light light;
        int defaultLightColor = this.mContext.getResources().getColor(17170726);
    ...
    public void setStatusBarCheckedImportance(boolean importance) {
        this.mStatusBarCheckedImportance = importance;
    }

    public boolean getStatusBarCheckedImportance() {
        return this.mStatusBarCheckedImportance;
    }
    ...
    public void dump(PrintWriter pw, String prefix, Context baseContext, boolean redact) {
        ...
        printWriter.println(prefix2 + "icon=" + iconStr);
        ...
        printWriter.println(prefix2 + "number=" + notification.number);
        printWriter.println(prefix2 + "groupAlertBehavior=" + notification.getGroupAlertBehavior());
        ...
        if (!TextUtils.isEmpty(notification.tickerText)) {
            ...
            if (redact) {
                printWriter.print(ticker.length() > 16 ? ticker.substring(0, 8) : "");
                printWriter.println("...");
            } else {
                printWriter.println(ticker);
            }
        } else {
        ...
        sb3.append(prefix2);
        char c2 = 1;
        sb3.append(String.format("color=0x%08x", new Object[]{Integer.valueOf(notification.color)}));
        ...
        printWriter.println(prefix2 + "mSystemImportance=" + NotificationListenerService.Ranking.importanceToString(this.mSystemImportance));
        printWriter.println(prefix2 + "mAsstImportance=" + NotificationListenerService.Ranking.importanceToString(this.mAssistantImportance));
        printWriter.println(prefix2 + "mImportance=" + NotificationListenerService.Ranking.importanceToString(this.mImportance));
        printWriter.println(prefix2 + "mImportanceExplanation=" + getImportanceExplanation());
        ...
        printWriter.println(prefix2 + "mInterruptionTimeMs=" + this.mInterruptionTimeMs);
        ...
        if (this.mPreChannelsNotification) {
            ...
            sb4.append(prefix2);
            sb4.append(String.format("  led=0x%08x onMs=%d offMs=%d", new Object[]{Integer.valueOf(notification.ledARGB), Integer.valueOf(notification.ledOnMS), Integer.valueOf(notification.ledOffMS)}));
        ...
        synchronized (this.mAdjustments) {
            for (Adjustment adjustment : this.mAdjustments) {
                ...
                if (signals.containsKey("key_importance")) {
                    setAssistantImportance(Math.min(4, Math.max(JobSchedulerShellCommand.CMD_ERR_NO_PACKAGE, signals.getInt("key_importance"))));
                }
                if (signals.isEmpty() == 0 && adjustment.getIssuer() != null) {
    ...
    private String getUserExplanation() {
        if (this.mUserExplanation == null) {
            this.mUserExplanation = this.mContext.getResources().getString(17040187);
        }
    ...
    public void setInterruptive(boolean interruptive) {
        ...
        if (interruptive) {
            MetricsLogger.action(getLogMaker().setCategory(1501).setType(1).addTaggedData(NetworkConstants.ETHER_MTU, Integer.valueOf(getInterruptionMs(now))));
    ...
    public LogMaker getLogMaker(long now) {
        LogMaker lm = this.sbn.getLogMaker().addTaggedData(858, Integer.valueOf(this.mImportance)).addTaggedData(793, Integer.valueOf(getLifespanMs(now))).addTaggedData(795, Integer.valueOf(getFreshnessMs(now))).addTaggedData(794, Integer.valueOf(getExposureMs(now))).addTaggedData(NetworkConstants.ETHER_MTU, Integer.valueOf(getInterruptionMs(now)));
        int i = this.mImportanceExplanationCode;
        if (i != 0) {
