...
public class SnoozeHelper {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, final Intent intent) {
            ...
            if (SnoozeHelper.REPOST_ACTION.equals(intent.getAction())) {
                NotificationManagerService.getHandler().post(new Runnable() {
                    public void run() {
    ...
    public boolean cancel(int userId, String pkg, String tag, int id) {
        ...
        if (!this.mSnoozedNotifications.containsKey(Integer.valueOf(userId)) || (recordsForPkg = (ArrayMap) this.mSnoozedNotifications.get(Integer.valueOf(userId)).get(pkg)) == null) {
            return false;
        }
