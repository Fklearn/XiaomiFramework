...
class IdmapManager {
    ...
    public boolean createIdmap(PackageInfo targetPackage, PackageInfo overlayPackage, int userId) {
        ...
        PackageInfo packageInfo2 = overlayPackage;
        int sharedAppGid = UserHandle.getSharedAppGid(packageInfo.applicationInfo.uid);
        String targetPath = packageInfo.applicationInfo.getBaseCodePath();
        String overlayPath = packageInfo2.applicationInfo.getBaseCodePath();
        try {
            int policies = calculateFulfilledPolicies(targetPackage, overlayPackage, userId);
            boolean enforce = enforceOverlayable(packageInfo2);
            try {
