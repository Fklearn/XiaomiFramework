...
public class ComponentResolver {
    ...
    /* access modifiers changed from: package-private */
    public List<ResolveInfo> queryServices(Intent intent, String resolvedType, int flags, List<PackageParser.Service> services2, int userId) {
        List<ResolveInfo> queryIntentForPackage;
        synchronized (this.mLock) {
            queryIntentForPackage = this.mServices.queryIntentForPackage(intent, resolvedType, flags, services2, userId);
        }
        return queryIntentForPackage;
    }
    ...
    private void addReceiversLocked(PackageParser.Package pkg, boolean chatty) {
        ...
        for (int i = 0; i < receiversSize; i++) {
            ...
            a.info.processName = PackageManagerService.fixProcessName(pkg.applicationInfo.processName, a.info.processName);
            this.mReceivers.addActivity(a, "receiver", (List<PackageParser.ActivityIntentInfo>) null);
        }
    }
    ...
    private void adjustPriority(List<PackageParser.Activity> systemActivities, PackageParser.ActivityIntentInfo intent, String setupWizardPackage) {
        ...
        if (intent.getPriority() > 0) {
            ...
            if (!((activityInfo.applicationInfo.privateFlags & 8) != 0)) {
                ...
                ArrayList findFilters = this.mActivities.findFilters(activityIntentInfo);
                ...
                String str2 = setupWizardPackage;
            } else if (this.mDeferProtectedFilters) {
                if (this.mProtectedFilters == null) {
                    this.mProtectedFilters = new ArrayList();
                }
                this.mProtectedFilters.add(activityIntentInfo);
            } else if (!activityIntentInfo.activity.info.packageName.equals(setupWizardPackage)) {
                activityIntentInfo.setPriority(0);
            }
        }
    }
    ...
    private void removeAllComponentsLocked(PackageParser.Package pkg, boolean chatty) {
        ...
        for (int i = 0; i < componentSize; i++) {
            this.mActivities.removeActivity((PackageParser.Activity) pkg.activities.get(i), "activity");
        }
        ...
        for (int i3 = 0; i3 < componentSize3; i3++) {
            this.mReceivers.removeActivity((PackageParser.Activity) pkg.receivers.get(i3), "receiver");
        }
        int componentSize4 = pkg.services.size();
        for (int i4 = 0; i4 < componentSize4; i4++) {
            this.mServices.removeService((PackageParser.Service) pkg.services.get(i4));
        }
    }
    ...
    private static final class ActivityIntentResolver extends IntentResolver<PackageParser.ActivityIntentInfo, ResolveInfo> {
        ...
        public ResolveInfo newResult(PackageParser.ActivityIntentInfo info, int match, int userId) {
            ...
            if (ps == null || (ai = PackageParser.generateActivityInfo(activity, this.mFlags, userState, userId)) == null) {
                return null;
            }
            ...
            if ((userState = ps.readUserState(userId)).instantApp && ps.isUpdateAvailable()) {
                return null;
            }
    ...
    private static final class ProviderIntentResolver extends IntentResolver<PackageParser.ProviderIntentInfo, ResolveInfo> {
        ...
        public ResolveInfo newResult(PackageParser.ProviderIntentInfo filter, int match, int userId) {
            ...
            if (!ComponentResolver.sPackageManagerInternal.isEnabledAndMatches(info.provider.info, this.mFlags, userId)) {
                return null;
            }
    ...
    private static final class ServiceIntentResolver extends IntentResolver<PackageParser.ServiceIntentInfo, ResolveInfo> {
        ...
        public ResolveInfo newResult(PackageParser.ServiceIntentInfo filter, int match, int userId) {
            ...
            if (!ComponentResolver.sPackageManagerInternal.isEnabledAndMatches(info.service.info, this.mFlags, userId)) {
                return null;
            }
            PackageParser.Service service = info.service;
            PackageSetting ps = (PackageSetting) service.owner.mExtras;
            if (ps == null || (si = PackageParser.generateServiceInfo(service, this.mFlags, userState, userId)) == null) {
                return null;
            }
            boolean isInstantApp = true;
            boolean matchVisibleToInstantApp = (this.mFlags & DumpState.DUMP_SERVICE_PERMISSIONS) != 0;
