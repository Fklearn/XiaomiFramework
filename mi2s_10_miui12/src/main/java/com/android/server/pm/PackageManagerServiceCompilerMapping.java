...
public class PackageManagerServiceCompilerMapping {
    static final int REASON_BOOTING_INDEX = 8;
    static final int REASON_SHARED_INDEX = 6;
    public static final String[] REASON_STRINGS = {"first-boot", "boot", "install", "bg-dexopt", "ab-ota", "inactive", "shared", "secondary", "booting"};

    static {
        String[] strArr = REASON_STRINGS;
        if (9 != strArr.length) {
            throw new IllegalStateException("REASON_STRINGS not correct");
        } else if (!"booting".equals(strArr[8])) {
