...
public final class DexoptOptions {
    ...
    public static final int DEXOPT_BG_CPUSET = 16384;
    ...
    public DexoptOptions(String packageName, String compilerFilter, int flags) {
        this(packageName, -1, compilerFilter, (String) null, flags);
    }

    public DexoptOptions(String packageName, int compilationReason, int flags) {
        this(packageName, compilationReason, PackageManagerServiceCompilerMapping.getCompilerFilterForReason(compilationReason), (String) null, flags);
    }

    public DexoptOptions(String packageName, int compilationReason, String compilerFilter, String splitName, int flags) {
        if (compilerFilter != null && compilerFilter.contains(DEXOPT_BG_TAG)) {
            compilerFilter = compilerFilter.substring(DEXOPT_BG_TAG.length());
            flags |= 16384;
        }
        if (((~18047) & flags) == 0) {
    ...
    public boolean isDexoptBgCpuset() {
