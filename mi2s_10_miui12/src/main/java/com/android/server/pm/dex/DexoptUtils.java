...
public final class DexoptUtils {
    ...
    public static String[] getClassLoaderContexts(ApplicationInfo info, List<SharedLibraryInfo> sharedLibraries, boolean[] pathsWithCode) {
        ...
        String baseApkContextClassLoader = encodeClassLoader("", applicationInfo.classLoaderName, sharedLibrariesContext);
        if (info.getSplitCodePaths() == null) {
            return new String[]{baseApkContextClassLoader};
        }
        String[] splitRelativeCodePaths = getSplitRelativeCodePaths(info);
        String baseClassPath = new File(info.getBaseCodePath()).getName();
        ...
        if (!info.requestsIsolatedSplitLoading() || applicationInfo.splitDependencies == null) {
            String classpath = baseClassPath;
            for (int i = 1; i < classLoaderContexts.length; i++) {
                if (pathsWithCode[i]) {
                    classLoaderContexts[i] = encodeClassLoader(classpath, applicationInfo.classLoaderName, sharedLibrariesContext);
                } else {
            ...
            String splitDependencyOnBase = encodeClassLoader(baseClassPath, applicationInfo.classLoaderName);
            ...
            for (int i4 = 1; i4 < classLoaderContexts.length; i4++) {
                String splitClassLoader = encodeClassLoader("", applicationInfo.splitClassLoaderNames[i4 - 1]);
                if (pathsWithCode[i4]) {
