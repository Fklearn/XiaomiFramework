...
public class BatterySaverStateMachine {
    ...
    public void setSettingsLocked(boolean batterySaverEnabled, boolean batterySaverEnabledSticky, int batterySaverTriggerThreshold, boolean isStickyAutoDisableEnabled, int stickyAutoDisableThreshold, int automaticBatterySaver, boolean dynamicPowerSavingsBatterySaver, int dynamicPowerSavingsDisableThreshold) {
        boolean z = batterySaverEnabled;
        boolean z2 = batterySaverEnabledSticky;
        int i = batterySaverTriggerThreshold;
        boolean z3 = isStickyAutoDisableEnabled;
        int i2 = automaticBatterySaver;
        boolean z4 = dynamicPowerSavingsBatterySaver;
        int i3 = dynamicPowerSavingsDisableThreshold;
        this.mSettingsLoaded = true;
        int stickyAutoDisableThreshold2 = Math.max(stickyAutoDisableThreshold, i);
        boolean enabledChanged = this.mSettingBatterySaverEnabled != z;
        boolean stickyChanged = this.mSettingBatterySaverEnabledSticky != z2;
        boolean thresholdChanged = this.mSettingBatterySaverTriggerThreshold != i;
        boolean stickyAutoDisableEnabledChanged = this.mSettingBatterySaverStickyAutoDisableEnabled != z3;
        ...
        boolean automaticModeChanged = this.mSettingAutomaticBatterySaver != i2;
        boolean dynamicPowerSavingsThresholdChanged = this.mDynamicPowerSavingsDisableThreshold != i3;
        boolean dynamicPowerSavingsBatterySaverChanged = this.mDynamicPowerSavingsBatterySaver != z4;
        if (enabledChanged || stickyChanged || thresholdChanged || automaticModeChanged || stickyAutoDisableEnabledChanged || stickyAutoDisableThresholdChanged || dynamicPowerSavingsThresholdChanged || dynamicPowerSavingsBatterySaverChanged) {
            ...
            this.mSettingAutomaticBatterySaver = i2;
            this.mDynamicPowerSavingsDisableThreshold = dynamicPowerSavingsDisableThreshold;
            this.mDynamicPowerSavingsBatterySaver = z4;
            if (thresholdChanged) {
    ...
    public void triggerDynamicModeNotification() {
        ...
        ensureNotificationChannelExists(manager, DYNAMIC_MODE_NOTIF_CHANNEL_ID, 17039932);
        manager.notifyAsUser(TAG, DYNAMIC_MODE_NOTIFICATION_ID, buildNotification(DYNAMIC_MODE_NOTIF_CHANNEL_ID, this.mContext.getResources().getString(17039934), 17039933, "android.intent.action.POWER_USAGE_SUMMARY"), UserHandle.ALL);
    }
    ...
    public void triggerStickyDisabledNotification() {
        NotificationManager manager = (NotificationManager) this.mContext.getSystemService(NotificationManager.class);
        ensureNotificationChannelExists(manager, BATTERY_SAVER_NOTIF_CHANNEL_ID, 17039621);
        String percentage = NumberFormat.getPercentInstance().format(((double) this.mBatteryLevel) / 100.0d);
        manager.notifyAsUser(TAG, STICKY_AUTO_DISABLED_NOTIFICATION_ID, buildNotification(BATTERY_SAVER_NOTIF_CHANNEL_ID, this.mContext.getResources().getString(17039618, new Object[]{percentage}), 17039623, "android.settings.BATTERY_SAVER_SETTINGS"), UserHandle.ALL);
    }
    ...
    private Notification buildNotification(String channelId, String title, int summaryId, String intentAction) {
        ...
        return new Notification.Builder(this.mContext, channelId).setSmallIcon(17302308).setContentTitle(title).setContentText(summary).setContentIntent(batterySaverIntent).setStyle(new Notification.BigTextStyle().bigText(summary)).setOnlyAlertOnce(true).setAutoCancel(true).build();
    }
