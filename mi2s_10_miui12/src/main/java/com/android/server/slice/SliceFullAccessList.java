...
public class SliceFullAccessList {
    ...
    public boolean hasFullAccess(String pkg, int userId) {
        ArraySet<String> pkgs = this.mFullAccessPkgs.get(userId, (Object) null);
    ...
    public void writeXml(XmlSerializer out, int user) throws IOException {
        out.startTag((String) null, TAG_LIST);
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (user == -1 || user == userId) {
                out.startTag((String) null, TAG_USER);
                out.attribute((String) null, TAG_USER, Integer.toString(userId));
                if (pkgs != null) {
                    int M = pkgs.size();
                    for (int j = 0; j < M; j++) {
                ...
                out.endTag((String) null, TAG_USER);
            }
        }
        out.endTag((String) null, TAG_LIST);
    }
