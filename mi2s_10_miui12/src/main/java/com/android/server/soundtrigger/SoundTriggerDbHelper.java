...
public class SoundTriggerDbHelper extends SQLiteOpenHelper {
    ...
    public boolean updateGenericSoundModel(SoundTrigger.GenericSoundModel soundModel) {
        ...
        synchronized (this) {
            ...
            try {
                z = db.insertWithOnConflict(GenericSoundModelContract.TABLE, (String) null, values, 5) != -1;
            } finally {
                db.close();
            }
        }
    ...
    public SoundTrigger.GenericSoundModel getGenericSoundModel(UUID model_uuid) {
        synchronized (this) {
            ...
            Cursor c = db.rawQuery("SELECT  * FROM st_sound_model WHERE model_uuid= '" + model_uuid + "'", (String[]) null);
            try {
                if (c.moveToFirst()) {
    ...
    public boolean deleteGenericSoundModel(UUID model_uuid) {
        synchronized (this) {
            ...
            SQLiteDatabase db = getWritableDatabase();
            try {
                if (db.delete(GenericSoundModelContract.TABLE, "model_uuid='" + soundModel.uuid.toString() + "'", (String[]) null) != 0) {
                    z = true;
                }
                return z;
            } finally {
