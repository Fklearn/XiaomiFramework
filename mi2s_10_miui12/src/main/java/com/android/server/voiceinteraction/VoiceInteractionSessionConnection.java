...
final class VoiceInteractionSessionConnection implements ServiceConnection, AssistDataRequester.AssistDataRequesterCallbacks {
    ...
    public VoiceInteractionSessionConnection(Object lock, ComponentName component, int user, Context context, Callback callback, int callingUid, Handler handler) {
        ...
        this.mAssistDataRequester = new AssistDataRequester(context3, this.mIWindowManager, (AppOpsManager) context3.getSystemService("appops"), this, this.mLock, 49, 50);
    ...
    public void onAssistDataReceivedLocked(Bundle data, int activityIndex, int activityCount) {
        ...
        if (iVoiceInteractionSession != null) {
            if (bundle == null) {
                ...
                int taskId = bundle.getInt(ActivityTaskManagerInternal.ASSIST_TASK_ID);
                IBinder activityId = bundle.getBinder(ActivityTaskManagerInternal.ASSIST_ACTIVITY_ID);
                Bundle assistData = bundle.getBundle("data");
                AssistStructure structure = (AssistStructure) bundle.getParcelable(ActivityTaskManagerInternal.ASSIST_KEY_STRUCTURE);
                AssistContent content = (AssistContent) bundle.getParcelable(ActivityTaskManagerInternal.ASSIST_KEY_CONTENT);
                if (assistData != null) {
    ...
    public void grantUriPermission(Uri uri, int mode, int srcUid, int destUid, String destPkg) {
        ...
        if (ActivityTaskManagerInternal.ASSIST_KEY_CONTENT.equals(uri.getScheme())) {
            ...
            try {
                ...
                Uri uri3 = ContentProvider.getUriWithoutUserId(uri);
                try {
                    UriGrantsManager.getService().grantUriPermissionFromOwner(this.mPermissionOwner, srcUid, destPkg, uri3, 1, sourceUserId, this.mUser);
    ...
    public boolean hideLocked() {
        ...
        if (this.mShown) {
            ...
            this.mUgmInternal.revokeUriPermissionFromOwner(this.mPermissionOwner, (Uri) null, 3, this.mUser);
