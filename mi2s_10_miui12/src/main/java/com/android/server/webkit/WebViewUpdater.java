...
class WebViewUpdater {
    ...
    static boolean isInstalledAndEnabledForAllUsers(List<UserPackage> userPackages) {
        for (UserPackage userPackage : userPackages) {
            if (!WebViewUpdaterInjector.isInstalledAndEnabledForUser(userPackage) && (!userPackage.isInstalledPackage() || !userPackage.isEnabledPackage())) {
                return false;
            }
        }
        return true;
    ...
    public void dumpState(PrintWriter pw) {
        synchronized (this.mLock) {
            ...
            pw.println(String.format("  Minimum targetSdkVersion: %d", new Object[]{29}));
    ...
    private void dumpAllPackageInformationLocked(PrintWriter pw) {
        ...
        for (WebViewProviderInfo provider : allProviders) {
            ...
            if (systemUserPackageInfo == null) {
                pw.println(String.format("    %s is NOT installed.", new Object[]{provider.packageName}));
            } else {
