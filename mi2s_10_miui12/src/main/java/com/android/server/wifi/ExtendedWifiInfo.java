...
public class ExtendedWifiInfo extends WifiInfo {
    ...
    private void update(int source, long txgood, long txretries, long txbad, long rxgood, long timeStamp) {
        ...
        long j5 = rxgood;
        long j6 = timeStamp;
        if (i == this.mLastSource) {
            long j7 = this.mLastPacketCountUpdateTimeStamp;
            if (j7 != RESET_TIME_STAMP && j7 < j6 && this.txBad <= j4 && this.txSuccess <= j2 && this.rxSuccess <= j5 && this.txRetries <= j3) {
