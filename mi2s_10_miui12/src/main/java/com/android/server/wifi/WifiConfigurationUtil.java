...
public class WifiConfigurationUtil {
    ...
    private static final int SSID_UTF_8_MAX_LEN = 50;
    ...
    public static boolean isConfigForSha256Network(WifiConfiguration config) {
        return config.allowedKeyManagement.get(13);
    }

    public static boolean isConfigForSha384Network(WifiConfiguration config) {
        return config.allowedKeyManagement.get(14);
    }
    ...
    private static boolean validateBitSets(WifiConfiguration config) {
        if (isConfigForWapiPskNetwork(config)) {
            Log.e(TAG, "validateBitsets : WAPI CERT " + config.allowedKeyManagement);
        } else if (isConfigForWapiCertNetwork(config)) {
            Log.e(TAG, "validateBitsets : WAPI CERT " + config.allowedKeyManagement);
        } else if (!validateBitSet(config.allowedKeyManagement, WifiConfiguration.KeyMgmt.strings.length)) {
            Log.e(TAG, "validateBitsets failed: invalid allowedKeyManagement bitset " + config.allowedKeyManagement);
            return false;
        }
        if (!validateBitSet(config.allowedProtocols, WifiConfiguration.Protocol.strings.length)) {
    ...
    private static boolean validateKeyMgmt(BitSet keyMgmnt) {
        if (keyMgmnt.cardinality() > 1) {
            if (keyMgmnt.cardinality() > 4) {
                ...
                return false;
            } else if (keyMgmnt.cardinality() == 3 && !keyMgmnt.get(10) && !keyMgmnt.get(13) && !keyMgmnt.get(14)) {
