...
class WifiDiagnostics extends BaseWifiDiagnostics {
    ...
    private static final int[] MinBufferSizes = {0, 16384, 16384, ISupplicantVendorStaNetwork.VendorKeyMgmtMask.IEEE8021X_SUITEB};
    ...
    public WifiDiagnostics(Context context, WifiInjector wifiInjector, WifiNative wifiNative, BuildProperties buildProperties, LastMileLogger lastMileLogger, Clock clock) {
        super(wifiNative);
        this.RING_BUFFER_BYTE_LIMIT_SMALL = context.getResources().getInteger(17694956) * 1024;
        this.RING_BUFFER_BYTE_LIMIT_LARGE = context.getResources().getInteger(17694957) * 1024;
        int[] fatalFirmwareAlertErrorCodeArray = context.getResources().getIntArray(17236086);
    ...
    public synchronized void startPacketLog() {
        if (this.mPerPacketRingBuffer != null) {
            ...
            this.mLog.tC("There is no per packet ring buffer");
        }
    }
    ...
    public synchronized void stopLogging() {
        if (this.mIsLoggingEventHandlerRegistered) {
            if (!this.mWifiNative.resetLogHandler()) {
                ...
                this.mLog.tC("Reset log handler");
            }
            this.mIsLoggingEventHandlerRegistered = false;
        }
        if (this.mLogLevel != 0) {
    ...
    public synchronized void onWifiAlert(int errorCode, byte[] buffer) {
        if (errorCode < 256) {
            ...
            int errorCode2 = errorCode - 256;
            captureAlertData(errorCode2, buffer);
            this.mWifiMetrics.logFirmwareAlert(errorCode2);
            Intent intent = new Intent("com.qualcomm.qti.net.wifi.WIFI_DATA_STALL");
            intent.putExtra("data_stall_reasoncode", errorCode2);
    ...
    private boolean fetchRingBuffers() {
        ...
        if (ringBufferStatusArr != null) {
            for (WifiNative.RingBufferStatus buffer : ringBufferStatusArr) {
                this.mLog.trace("RingBufferStatus is: %").c(buffer.name).flush();
    ...
    private boolean startLoggingAllExceptPerPacketBuffers() {
        WifiNative.RingBufferStatus[] ringBufferStatusArr = this.mRingBuffers;
        if (ringBufferStatusArr == null) {
            this.mLog.tC("No ring buffers to log anything!");
            return false;
        }
        for (WifiNative.RingBufferStatus buffer : ringBufferStatusArr) {
            if ((buffer.flag & 4) != 0) {
                this.mLog.trace("skipped per packet logging ring %").c(buffer.name).flush();
            } else {
    ...
    private boolean startLoggingRingBuffer(WifiNative.RingBufferStatus buffer) {
        ...
        this.mLog.warn("Could not start logging ring %").c(buffer.name).flush();
        return false;
    }
    ...
    private boolean stopLoggingRingBuffer(WifiNative.RingBufferStatus buffer) {
        ...
        this.mLog.warn("Could not stop logging ring %").c(buffer.name).flush();
        return true;
    }
    ...
    private BugReport captureBugreport(int errorCode, boolean captureFWDump) {
        ...
        this.mLog.warn("CaptureBugReport %").c((long) errorCode).flush();
    ...
    public String compressToBase64(byte[] input) {
        ...
        try {
            ...
            this.mLog.dump("length is: %").c(compressed == null ? 0 : (long) compressed.length).flush();
            String result = Base64.encodeToString(compressed.length < input.length ? compressed : input, 0);
            this.mLog.dump("FwMemoryDump length is: %").c((long) result.length()).flush();
            return result;
        } catch (IOException e) {
    ...
    private LimitedCircularArray<String> getKernelLog(int maxLines) {
        this.mLog.tC("Reading kernel log ...");
        ...
        this.mLog.dump("Added % lines").c((long) logLines.length).flush();
        return lines;
    }
