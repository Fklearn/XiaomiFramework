...
public class WrongPasswordNotifier {
    ...
    private final NotificationManager mNotificationManager = ((NotificationManager) this.mContext.getSystemService(ThemeManagerConstants.COMPONENT_CODE_NOTIFICATION));
    ...
    private void showNotification(String ssid) {
        ...
        this.mNotificationManager.notify(42, this.mFrameworkFacade.makeNotificationBuilder(this.mContext, SystemNotificationChannels.NETWORK_ALERTS).setAutoCancel(true).setTimeoutAfter(CANCEL_TIMEOUT_MILLISECONDS).setSmallIcon(17303569).setContentTitle(this.mContext.getString(17041397)).setContentText(ssid).setContentIntent(this.mFrameworkFacade.getActivity(this.mContext, 0, intent, MiuiWindowManager.LayoutParams.PRIVATE_FLAG_LOCKSCREEN_DISPALY_DESKTOP)).setColor(this.mContext.getResources().getColor(17170460)).build());
    }
