...
public class InformationElementUtil {
    ...
    public static class Capabilities {
        ...
        private static final int WPA2_AKM_DPP = 43675472;
        ...
        private void parseWapiElement(ScanResult.InformationElement ie) {
            ...
            try {
                ...
                for (int i = 0; i < akmCount; i++) {
                    int akm = buf.getInt();
                    if (akm == 24253440) {
                        wapiKeyManagement.add(17);
                    } else if (akm != WAPI_AKM_PSK) {
                        Log.w("IE_Capabilities", "Unknown WAPI AKM: " + Integer.toHexString(akm));
                    } else {
                        wapiKeyManagement.add(16);
                    }
                }
                wapiKeyManagement.isEmpty();
            ...
            if (ies != null && beaconCap != null) {
                ...
                for (ScanResult.InformationElement ie : ies) {
                    ...
                    if (ie.id == 45) {
                        ...
                        this.isVht = true;
                    } else if (ie.id == 255 && ie.bytes != null && ie.bytes.length > 0 && (ie.bytes[0] & 255) == 35) {
                        this.isHe = true;
                    }
                }
            }
        ...
        private String generateCapabilitiesStringPerProtocol(int index) {
            ...
            if (index < this.keyManagement.size()) {
                ...
                while (j < this.keyManagement.get(index).size()) {
                    capability.append(j == 0 ? "-" : CountryCode.GSM_GENERAL_IDD_CODE);
            ...
            if (index < this.pairwiseCipher.size()) {
                ...
                while (j2 < this.pairwiseCipher.get(index).size()) {
                    capability.append(j2 == 0 ? "-" : CountryCode.GSM_GENERAL_IDD_CODE);
        ...
        private String generateWPA2CapabilitiesString(String cap, int index) {
            ...
            if (index < this.keyManagement.size()) {
                int j = 0;
                while (j < this.keyManagement.get(index).size()) {
                    capWpa2.append(j == 0 ? "-" : CountryCode.GSM_GENERAL_IDD_CODE);
            ...
            if (index < this.pairwiseCipher.size()) {
                int j2 = 0;
                while (j2 < this.pairwiseCipher.get(index).size()) {
                    capWpa2.append(j2 == 0 ? "-" : CountryCode.GSM_GENERAL_IDD_CODE);
        ...
        public static int getRateFromByte(int byteVal) {
            switch (byteVal & 127) {
                ...
                case ISupplicantStaIfaceCallback.StatusCode.REJECT_DSE_BAND:
                    return 48000000;
                case MiBleProfile.PROPERTY_MI_BAND_EVENT /*108*/:
                    return 54000000;
                default:
                    return -1;
            }
