...
public class BoundsAnimationController {
    ...
    private static final int WAIT_FOR_DRAW_TIMEOUT_MS = 425;
    ...
    private final class AppTransitionNotifier extends WindowManagerInternal.AppTransitionListener implements Runnable {
        ...
        public void run() {
            for (int i = 0; i < BoundsAnimationController.this.mRunningAnimations.size(); i++) {
                ((BoundsAnimator) BoundsAnimationController.this.mRunningAnimations.valueAt(i)).onAnimationEnd((Animator) null);
            }
        }
    }
    ...
    final class BoundsAnimator extends ValueAnimator implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
        ...
        public void pause() {
            super.pause();
            BoundsAnimationController.this.mHandler.postDelayed(this.mResumeRunnable, 425);
        }
    ...
    public BoundsAnimator animateBoundsImpl(BoundsAnimationTarget target, Rect from, Rect to, int animationDuration, int schedulePipModeChangedState, boolean moveFromFullscreen, boolean moveToFullscreen, @AnimationType int animationType) {
        ...
        BoundsAnimator existing = this.mRunningAnimations.get(boundsAnimationTarget);
        ...
        } else if (existing.isAnimatingTo(rect) && ((!moveToFullscreen || existing.mMoveToFullscreen) && (!moveFromFullscreen || existing.mMoveFromFullscreen))) {
            return existing;
        } else {
        ...
        if (animationType2 == 1) {
            boundsAnimationTarget.setPinnedStackSize(rect, (Rect) null);
        }
