...
final class InputMonitor {
    ...
    private static final class EventReceiverInputConsumer extends InputConsumerImpl implements WindowManagerPolicy.InputConsumer {
        ...
        EventReceiverInputConsumer(WindowManagerService service, InputMonitor monitor, Looper looper, String name, InputEventReceiver.Factory inputEventReceiverFactory, int clientPid, UserHandle clientUser, int displayId) {
            super(service, (IBinder) null, name, (InputChannel) null, clientPid, clientUser, displayId);
            this.mInputMonitor = monitor;
            Looper looper2 = looper;
            InputEventReceiver.Factory factory = inputEventReceiverFactory;
    ...
    public WindowManagerPolicy.InputConsumer createInputConsumer(Looper looper, String name, InputEventReceiver.Factory inputEventReceiverFactory) {
        if (!this.mInputConsumers.containsKey(name)) {
            EventReceiverInputConsumer eventReceiverInputConsumer = new EventReceiverInputConsumer(this.mService, this, looper, name, inputEventReceiverFactory, Process.myPid(), UserHandle.SYSTEM, this.mDisplayId);
    ...
    public void createInputConsumer(IBinder token, String name, InputChannel inputChannel, int clientPid, UserHandle clientUser) {
        if (!this.mInputConsumers.containsKey(name)) {
            InputConsumerImpl inputConsumerImpl = new InputConsumerImpl(this.mService, token, name, inputChannel, clientPid, clientUser, this.mDisplayId);
            char c = 65535;
            int hashCode = name.hashCode();
            if (hashCode != 1024719987) {
                if (hashCode != 1415830696) {
                    if (hashCode == 1848302522 && name.equals(MiuiFreeFormGestureController.FREEFORM_INPUT_CONSUMER)) {
                        c = 2;
                    }
                } else if (name.equals("wallpaper_input_consumer")) {
                    c = 0;
                }
    ...
    public void forceUpdateImmediately() {
        ...
        this.mUpdateInputWindowsPending = false;
    }
    ...
    private final class UpdateInputForAllWindowsConsumer implements Consumer<WindowState> {
        InputConsumerImpl freeformInputConsumer;
        InputConsumerImpl gestureAnimationInputConsumer;
        boolean inDrag;
        private boolean mAddFreeFormInputConsumerHandle;
        private boolean mAddGestureInputConsumerHandle;
        ...
        public void updateInputWindows(boolean inDrag2) {
            ...
            if (InputMonitor.this.mDisplayId == 0) {
                this.gestureAnimationInputConsumer = InputMonitor.this.getInputConsumer("gesture_input_consumer");
                this.freeformInputConsumer = InputMonitor.this.getInputConsumer(MiuiFreeFormGestureController.FREEFORM_INPUT_CONSUMER);
            }
            ...
            this.mAddGestureInputConsumerHandle = this.gestureAnimationInputConsumer != null;
            this.mAddFreeFormInputConsumerHandle = this.freeformInputConsumer != null;
        ...
        public void accept(WindowState w) {
            ...
            if (inputChannel != null && inputWindowHandle != null && !windowState.mRemoved && !w.cantReceiveTouchInput()) {
                ...
                boolean ignoreInput = windowState.mAppToken != null && windowState.mAppToken.mIgnoreInput;
                if (windowState.mAppToken != null && windowState.mAppToken.mIsCastMode) {
                    z = false;
                } else {
                    z = w.isVisibleLw() && !MiuiGestureController.isWindowDummyVisible(w) && !ignoreInput;
                }
                boolean isVisible = z;
                if (this.mAddGestureInputConsumerHandle) {
                    this.gestureAnimationInputConsumer.show(InputMonitor.this.mInputTransaction, windowState);
                    this.mAddGestureInputConsumerHandle = false;
                }
                ...
                if (w.inFreeformWindowingMode() && this.mAddFreeFormInputConsumerHandle) {
                    Rect smallFreeFormtouchableRegion = InputMonitor.this.mService.mMiuiFreeFormGestureController.getSmallFreeFormWindowBounds();
                    if (InputMonitor.this.mService.getCurrentFreeFormWindowMode() == 1) {
                        this.freeformInputConsumer.mWindowHandle.touchableRegion.set(smallFreeFormtouchableRegion);
                    } else {
                        windowState.getSurfaceTouchableRegion(this.freeformInputConsumer.mWindowHandle, flags);
                    }
                    if (!w.isOnScreen() || windowState.mWinAnimator == null || !windowState.mWinAnimator.getShown()) {
                        this.freeformInputConsumer.mWindowHandle.visible = false;
                    } else {
                        this.freeformInputConsumer.mWindowHandle.visible = true;
                    }
                    this.freeformInputConsumer.show(InputMonitor.this.mInputTransaction, windowState);
                    this.mAddFreeFormInputConsumerHandle = false;
                }
                ...
                if (this.mAddWallpaperInputConsumerHandle && windowState.mAttrs.type == 2013 && w.isVisibleLw()) {
            ...
            } else if (windowState.mWinAnimator.hasSurface()) {
