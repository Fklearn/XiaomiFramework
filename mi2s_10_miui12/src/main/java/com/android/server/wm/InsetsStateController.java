...
class InsetsStateController {
    ...
    public InsetsSourceProvider getSourceProvider(int type) {
        return (InsetsSourceProvider) this.mControllers.computeIfAbsent(Integer.valueOf(type), new Function() {
    ...
    private void addToControlMaps(WindowState win, int type) {
        ((ArrayList) this.mWinControlTypeMap.computeIfAbsent(win, $$Lambda$InsetsStateController$Ofxsu0zvrpKfv2Cf4dVk1yXm2uI.INSTANCE)).add(Integer.valueOf(type));
