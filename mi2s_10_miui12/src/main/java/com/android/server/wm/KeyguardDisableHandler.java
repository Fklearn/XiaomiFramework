...
class KeyguardDisableHandler {
    ...
    private boolean shouldKeyguardBeEnabled(int userId) {
        ...
        if (!(((!dpmRequiresPassword && !this.mInjector.isKeyguardSecure(this.mCurrentUser)) && this.mAppTokenWatcher.isAcquired(userId)) || ((dpmRequiresPassword ^ true) && this.mSystemTokenWatcher.isAcquired(userId)))) {
            return true;
        }
        return false;
    }
