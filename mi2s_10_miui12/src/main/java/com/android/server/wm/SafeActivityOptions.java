...
public class SafeActivityOptions {
    ...
    public ActivityOptions getOptions(ActivityStackSupervisor supervisor) throws SecurityException {
        return getOptions((Intent) null, (ActivityInfo) null, (WindowProcessController) null, supervisor);
    }
    ...
    private void checkPermissions(Intent intent, ActivityInfo aInfo, WindowProcessController callerApp, ActivityStackSupervisor supervisor, ActivityOptions options, int callingPid, int callingUid) {
        ...
        int i = callingPid;
        int i2 = callingUid;
        if (options.getLaunchTaskId() == -1 || activityStackSupervisor.mRecentTasks.isCallerRecents(i2) || ActivityTaskManagerService.checkPermission("android.permission.START_TASKS_FROM_RECENTS", i, i2) != -1) {
            ...
            if (activityInfo == null || launchDisplayId == -1 || activityStackSupervisor.isCallerAllowedToLaunchOnDisplay(i, i2, launchDisplayId, activityInfo)) {
                ...
                if (activityInfo != null && lockTaskMode && !activityStackSupervisor.mService.getLockTaskController().isPackageWhitelisted(UserHandle.getUserId(callingUid), activityInfo.packageName)) {
                    String msg = "Permission Denial: starting " + getIntentString(intent) + " from " + windowProcessController + " (pid=" + i + ", uid=" + i2 + ") with lockTaskMode=true";
                    Slog.w(TAG, msg);
                    throw new SecurityException(msg);
                } else if (options.getRemoteAnimationAdapter() != null) {
                    ActivityTaskManagerService activityTaskManagerService = activityStackSupervisor.mService;
                    if (ActivityTaskManagerService.checkPermission("android.permission.CONTROL_REMOTE_APP_TRANSITION_ANIMATIONS", i, i2) != 0) {
                        String msg2 = "Permission Denial: starting " + getIntentString(intent) + " from " + windowProcessController + " (pid=" + i + ", uid=" + i2 + ") with remoteAnimationAdapter";
                        Slog.w(TAG, msg2);
                        throw new SecurityException(msg2);
                    }
                }
            } else {
                String msg3 = "Permission Denial: starting " + getIntentString(intent) + " from " + windowProcessController + " (pid=" + i + ", uid=" + i2 + ") with launchDisplayId=" + launchDisplayId;
                Slog.w(TAG, msg3);
                throw new SecurityException(msg3);
            }
        } else {
            String msg4 = "Permission Denial: starting " + getIntentString(intent) + " from " + windowProcessController + " (pid=" + i + ", uid=" + i2 + ") with launchTaskId=" + options.getLaunchTaskId();
            Slog.w(TAG, msg4);
