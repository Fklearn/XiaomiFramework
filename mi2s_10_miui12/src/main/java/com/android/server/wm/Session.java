...
class Session extends IWindowSession.Stub implements IBinder.DeathRecipient {
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        try {
            ...
            if (!(e instanceof SecurityException)) {
                Slog.wtf(DisplayPolicy.TAG, "Window Session Crash", e);
            }
            throw e;
        }
    }
    ...
    public int addToDisplayWithoutInputChannel(IWindow window, int seq, WindowManager.LayoutParams attrs, int viewVisibility, int displayId, Rect outContentInsets, Rect outStableInsets, InsetsState outInsetsState) {
        return this.mService.addWindow(this, window, seq, attrs, viewVisibility, displayId, new Rect(), outContentInsets, outStableInsets, (Rect) null, new DisplayCutout.ParcelableWrapper(), (InputChannel) null, outInsetsState);
    }
    ...
    public void notifyHasSurfaceView(IBinder window, boolean hasSurfaceView) {
        long identity = Binder.clearCallingIdentity();
        try {
            this.mService.notifyHasSurfaceView(this, window, hasSurfaceView);
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
    }
        ...
        if (this.mNumWindow <= 0 && this.mClientDead) {
            ...
            if (surfaceSession != null) {
                try {
                    ...
                    Slog.w(DisplayPolicy.TAG, "Exception thrown when killing surface session " + this.mSurfaceSession + " in session " + this + ": " + e.toString());
                }
