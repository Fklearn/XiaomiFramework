...
class Task extends WindowContainer<AppWindowToken> implements ConfigurationContainerListener {
    ...
    boolean mUpdateTaskSnapshotByGesture;
    final int mUserId;
    ...
    public void addChild(AppWindowToken wtoken, int position) {
        ...
        if ((this.mStack.getMiuiConfigFlag() & 2) != 0 && wtoken != null) {
            wtoken.setMiuiConfigFlag(this.mStack.getMiuiConfigFlag(), false);
        }
    }
    ...
    public void onParentChanged() {
        ...
        for (int appTokenNdx = 0; appTokenNdx < this.mChildren.size(); appTokenNdx++) {
            ((AppWindowToken) this.mChildren.get(appTokenNdx)).onParentChanged();
        }
    }
    ...
    public void alignToAdjustedBounds(Rect adjustedBounds, Rect tempInsetBounds, boolean alignBottom) {
        if (isResizeable() && !Configuration.EMPTY.equals(getRequestedOverrideConfiguration())) {
            ...
            if (tempInsetBounds == null || tempInsetBounds.isEmpty()) {
                setOverrideDisplayedBounds((Rect) null);
    ...
    private void adjustBoundsForDisplayChangeIfNeeded(DisplayContent displayContent) {
        ...
        if (displayContent != null) {
            ...
            if (displayId != this.mLastRotationDisplayId) {
                ...
                } else if (!MiuiMultiWindowUtils.mIsMiniFreeformMode) {
                    Rect rect = this.mTaskRecord.getBounds();
                    Slog.d("WindowManager", "Task::adjustBoundsForDisplayChangeIfNeeded::rect = " + rect);
                    boolean z = false;
                    if (displayContent.getRotation() == 1 || displayContent.getRotation() == 3) {
                        this.mTmpRect2 = MiuiMultiWindowUtils.getFreeformRect(this.mWmService.mContext, true, false, MiuiMultiWindowUtils.mIsMiniFreeformMode, rect.width() > rect.height());
                    } else {
                        Context context = this.mWmService.mContext;
                        boolean z2 = MiuiMultiWindowUtils.mIsMiniFreeformMode;
                        if (rect.width() > rect.height()) {
                            z = true;
                        }
                        this.mTmpRect2 = MiuiMultiWindowUtils.getFreeformRect(context, true, true, z2, z);
                    }
                }
    ...
    public AppWindowToken getTopVisibleAppToken() {
        for (int i = this.mChildren.size() - 1; i >= 0; i--) {
            AppWindowToken token = (AppWindowToken) this.mChildren.get(i);
            if (!token.mIsExiting && !token.isClientHidden() && !token.hiddenRequested && !MiuiGestureController.isAppDummyVisible(token)) {
                return token;
            }
        }
        return null;
