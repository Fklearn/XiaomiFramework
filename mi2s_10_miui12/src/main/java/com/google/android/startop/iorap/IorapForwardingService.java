...
public class IorapForwardingService extends SystemService {
    ...
    private class AppLaunchObserver implements ActivityMetricsLaunchObserver {
        ...
        public void onIntentStarted(Intent intent) {
            ...
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("AppLaunchObserver#onIntentStarted(%d, %s)", new Object[]{Long.valueOf(this.mSequenceId), intent}));
            }
        ...
        public void onIntentFailed() {
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("AppLaunchObserver#onIntentFailed(%d)", new Object[]{Long.valueOf(this.mSequenceId)}));
            }
        ...
        public void onActivityLaunched(byte[] activity, int temperature) {
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("AppLaunchObserver#onActivityLaunched(%d, %s, %d)", new Object[]{Long.valueOf(this.mSequenceId), activity, Integer.valueOf(temperature)}));
            }
        ...
        public void onActivityLaunchCancelled(byte[] activity) {
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("AppLaunchObserver#onActivityLaunchCancelled(%d, %s)", new Object[]{Long.valueOf(this.mSequenceId), activity}));
            }
        ...
        public void onActivityLaunchFinished(byte[] activity) {
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("AppLaunchObserver#onActivityLaunchFinished(%d, %s)", new Object[]{Long.valueOf(this.mSequenceId), activity}));
            }
            IorapForwardingService.invokeRemote(new RemoteRunnable(activity) {
                private final /* synthetic */ byte[] f$1;
    ...
    private class RemoteTaskListener extends ITaskListener.Stub {
        ...
        public void onProgress(RequestId requestId, TaskResult result) throws RemoteException {
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("RemoteTaskListener#onProgress(%s, %s)", new Object[]{requestId, result}));
            }
        }

        public void onComplete(RequestId requestId, TaskResult result) throws RemoteException {
            if (IorapForwardingService.DEBUG) {
                Log.v(IorapForwardingService.TAG, String.format("RemoteTaskListener#onComplete(%s, %s)", new Object[]{requestId, result}));
            }
        }
    }
